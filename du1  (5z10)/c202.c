...
void Stack_Init( Stack *stack ) {
    ...
    if (stack == NULL){             // ak stack == NULL, spustime funkciu Stack_Error
        Stack_Error(SERR_INIT);
    }else (stack->topIndex = -1);   // inak nastavime zasobniku hodnotu -1 (je prazdny)
...
int Stack_IsEmpty( const Stack *stack ) {
    ...
    return stack->topIndex == -1; //nastavime zasobniku hodnotu -1 (je prazdny)
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    if (Stack_IsEmpty(stack)){      // ak je prazdny zasobnik, spustime funkciu Stack_Error
        Stack_Error(SERR_TOP);
    }else (*dataPtr = stack->array[stack->topIndex]);   // do *dataPtr dam hodnotu na vrchu zasobniku
...
void Stack_Pop( Stack *stack ) {
    ...
    if (stack != NULL){     // ak je zasobnik prazdny nemam co mazat, nemusim vykonavat funkciu
        stack->topIndex--;  // zmensenim zasobniku odstranim prvok na vrcholu zasobniku
    }
...
void Stack_Push( Stack *stack, char data ) {
    ...
    if (Stack_IsFull(stack)){       // ak je zasobnik plny spustime funkciu Stack_Error
        Stack_Error(SERR_PUSH);
    }

    stack->topIndex++;              // zvacsim zasobnik a pridam data na zvysenu hodnotu
